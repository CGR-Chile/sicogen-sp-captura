...
$(document).ready(function () {
	...
	$('#proyecto').attr('maxlength','20');
	$('#denomProy').attr('maxlength','80');
		...
		$.ajax({url: 'saveLineFile.action',
		    ...
		    data:{
		    	...
		    	detLine:$('#txtArea').val()+
		    			...
		    			$('#entRegion').val()+
		    			$.strPadRight($('#proyecto').val().substring(0, 20),20,' ')+
		    			$.strPadRight($('#denomProy').val().substring(0, 80),80,' ')+
		    			'CLP'+
		    			$('#txtCodigo').val()+
			    	...
			    	$('#row'+data.linea.lineaId).find('td#dtBotones #edt'+data.linea.lineaId).click(function (){
			    		jConfirm('&iquest;Est&aacute; seguro que desea editar el registro?', 'Editar Registro', function(r) {
							if(r){
					    		...
					    		$('#proyecto').val(data.linea.linea.substring(30,49));
					    		$('#denomProy').val(data.linea.linea.substring(50,129));
					    		
					    		$('#txtCodigo').val(data.linea.linea.substring(133,143));
					    		$('#txtDenominacion').val(data.linea.linea.substring(143,223));
					    		$('#txtAumento').val(data.linea.linea.substring(223,239));
					    		$('#txtDisminucion').val(data.linea.linea.substring(239,255));
					    		if(!$.isNumeric(data.linea.linea.substring(223,239) )){
					    			tempAum=0;
					    		}else{
					    			tempAum=data.linea.linea.substring(223,239);
					    		}
					    		if(!$.isNumeric(data.linea.linea.substring(239,255) )){
					    			tempDis=0;
					    		}else{
					    			tempDis=data.linea.linea.substring(239,255);
					    		}
							}
			    		});
		...
		if(data.file.archivoId>0){
	    	...
	    	$.each(data.file.detalle, function(i,rw){
		    	...
		    	$("#ListaItem tbody").append('<tr id="row'+rw.lineaId+'" class="regDetalle '+clase+'">'+							
						...
						'<td id="dtProyecto" >'+rw.linea.substring(30,49)+'</td>'+
						'<td id="dtDenomProy" >'+rw.linea.substring(50,129)+'</td>'+
						'<td id="dtCuenta" >'+rw.linea.substring(133,143)+'</td>'+
						'<td id="dtDenomin" style="text-align:left;">'+rw.linea.substring(143,223)+'</td>'+
						'<td id="dtAumento" >'+rw.linea.substring(223,239)+'</td>'+
						'<td id="dtDismin" >'+rw.linea.substring(239,255)+'</td>'+
		    	...
		    	$('#row'+rw.lineaId).find('td#dtBotones #edt'+rw.lineaId).click(function (){
		    		jConfirm('&iquest;Est&aacute; seguro que desea editar el registro?', 'Editar Registro', function(r) {
						if(r){
				    		...
				    		$('#proyecto').val(rw.linea.substring(30,49));
				    		$('#denomProy').val(rw.linea.substring(50,129));
				    		
				    		$('#txtCodigo').val(rw.linea.substring(133,143));
				    		$('#txtDenominacion').val(rw.linea.substring(143,223));
				    		$('#txtAumento').val(rw.linea.substring(223,239));
				    		$('#txtDisminucion').val(rw.linea.substring(239,255));
				    		if(!$.isNumeric(rw.linea.substring(223,239) )){
				    			tempAum=0;
				    		}else{
				    			tempAum=rw.linea.substring(223,239);
				    		}
				    		if(!$.isNumeric(rw.linea.substring(239,255) )){
				    			tempDis=0;
				    		}else{
				    			tempDis=rw.linea.substring(239,255);
				    		}
						}
		    		});
