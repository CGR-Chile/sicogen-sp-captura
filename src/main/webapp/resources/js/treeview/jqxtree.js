	...
	a.extend(
		a.jqx._jqxTree.prototype,{
			defineInstance:function(){
				...
				this.aria={"aria-activedescendant":{name:"getActiveDescendant",type:"string"},"aria-disabled":{name:"disabled",type:"boolean"}}},
			createInstance:function(c){
				var b=this;this.host.attr("role","tree");
				this.host.attr("data-role","treeview");
				this.propertyChangeMap.disabled=function(f,h,g,j){
					if(b.disabled){b.host.addClass(b.toThemeProperty("jqx-tree-disabled"))}
					...
					if(this.element.innerHTML.indexOf("UL")){
						...
						if(e.length>0){
							this.createTree(e[0]);this.createdTree=true}}
					if(this.source!=null){
						var d=this.loadItems(this.source);
						this.element.innerHTML=d;var e=this.host.find("ul:first");
						if(e.length>0){this.createTree(e[0]);this.createdTree=true}}
			...
			checkItems:function(f,h){
				var e=this;
				if(f!=null){
					var d=0;var g=false;var b=0;
					var j=a(f.element).find("li");b=j.length;
					a.each(j,function(k){
						var l=e.itemMapping["id"+this.id].item;
						if(l.checked!=false){
							if(l.checked==null){g=true}d++;}
					});
					if(f!=h){
						if(d==b){
					...
					function(d){
						...
						if(b.keyboardNavigation||b.enableKeyboardNavigation){
							if(b.selectedItem!=null){
								var g=b.selectedItem.element;
								switch(l){
									case 32:
										if(b.checkboxes){
											b.fromKey=true;
											var k=a(b.selectedItem.checkBoxElement).jqxCheckBox("checked");
											b.checkItem(b.selectedItem.element,!k,"tree");
											if(b.hasThreeStates){
										...
										return true;
									case 33:
										var f=b._getItemsOnPage();
										var j=b.selectedItem;
										for(var h=0;h<f;h++){
											j=b._prevVisibleItem(j);}
										if(j!=null){
											b.selectItem(j.element);
											b.ensureVisible(j.element);
										}else{
									...
									case 35:
										...
										return false;
									case 38:
										var j=b._prevVisibleItem(b.selectedItem);
										if(j!=null){
											b.selectItem(j.element);
											b.ensureVisible(j.element)}
											return false;
			...
			_isVisible:function(c){
				...
				if(!this._isElementVisible(c.element)){return false}
				var b=this._parentItem(c);
				if(b==null){return true}
				if(b!=null){
					if(!this._isElementVisible(b.element)){return false}
					if(b.isExpanded){while(b!=null){
					b=this._parentItem(b);
					if(b!=null&&!this._isElementVisible(b.element)){return false}
					if(b!=null&&!b.isExpanded){return false}}}
					else{return false}
				}return true
			},
			_getItemsOnPage:function(){
				...
				if(parseInt(a(e.element).height())>0){
					while(f<=b){f+=parseInt(a(e.element).outerHeight());d++}
				}return d},
			...
			_parseItem:function(m){
				...
				var k=m.label;
				if(!m.label&&m.html){k=m.html}
				if(!k){k="Item"}
				if(typeof m==="string"){k=m}
				var h=false;
				if(m.expanded!=undefined&&m.expanded){h=true}
				var f=false;
				if(m.locked!=undefined&&m.locked){f=true}
				var d=false;if(m.selected!=undefined&&m.selected){d=true}
				var e=false;if(m.disabled!=undefined&&m.disabled){e=true}
				var l=false;if(m.checked!=undefined&&m.checked){l=true}
				var j=m.icon;
				var c=m.iconsize;g+="<li";
				if(h){g+=' item-expanded="true" '}
				if(f){g+=' item-locked="true" '}
				if(e){g+=' item-disabled="true" '}
				if(d){g+=' item-selected="true" '}
				if(c){g+=' item-iconsize="'+m.iconsize+'" '}
				if(j!=null&&j!=undefined){g+=' item-icon="'+j+'" '}
				if(m.label&&!m.html){g+=' item-label="'+k+'" '}
				if(m.value!=null){g+=' item-value="'+m.value+'" '}
				if(m.checked!=undefined){g+=' item-checked="'+l+'" '}
				var b="";
				if(m.id!=undefined){b=m.id;g+=' id="'+b+'" '}
				else{
					b=this.createID();
					g+=' id="'+b+'" '}
				g+=">"+k;
				if(m.items){g+=this.loadItems(m.items)}
				if(!this._valueList){this._valueList=new Array()}
				this._valueList[b]=m.value;g+="</li>";
				return g
			},
			...
			hitTest:function(h,g){
				...
				var b=this;
				var f=null;
				var e=this.host.find(".draggable");
				this._syncItems(e);
				if(b._visibleItems){
				...
				if(r&&r.treeInstance!=undefined){
					...
					var e=function(j){
						...
						if(p.selectedItem!=null){
							...
							p._render();
							this._raiseEvent("4",{items:this.getItems()});
							if(p.checkboxes){p._updateCheckLayout(null)}
			},
			addTo:function(q,s,p){
				var n=this;
				var o=new Array();
				if(s&&s.treeInstance!=undefined){s=s.element}
				if(!a.isArray(q)){o[0]=q}
				else{o=q}
				var k="";
				var r=this;
				a.each(o,function(){
				...
				if(n.element.innerHTML.indexOf("UL")){
					...
					if(e.length==0){
						...
						if(r.rtl){e.addClass(n.toThemeProperty("jqx-tree-dropdown-rtl"))}
						e.append(b);
						...
						var f=function(j){
							...
							if(r.selectedItem!=null){
								...
								f(false);
								r._render();
								if(r.checkboxes){r._updateCheckLayout(null)}
								this._raiseEvent("4",{items:this.getItems()})
			},
			updateItem:function(d,g){
				var f=d.treeInstance!=undefined?d:this.getItem(d);
				if(!f){var c=d;d=g;g=c;var f=d.treeInstance!=undefined?d:this.getItem(d)}
				if(f){
					if(typeof(g)==="string"){g={label:g}}
					if(g.value){f.value=g.value}
					if(g.label){f.label=g.label;a.jqx.utilities.html(a(f.titleElement),g.label)}
					if(g.icon){
						if(a(f.element).children(".itemicon").length>0){a(f.element).find(".itemicon")[0].src=g.icon}
						else{
							var b=g.iconsize;
							if(!b){b=16}
							var e=a('<img width="'+b+'" height="'+b+'" style="float: left;" class="itemicon" src="'+g.icon+'"/>');
							a(f.titleElement).prepend(e);
							e.css("margin-right","4px");
							if(this.rtl){e.css("margin-right","0px");e.css("margin-left","4px");e.css("float","right")}
						}
					}
					if(g.expanded){this.expandItem(f)}
					if(g.disabled){this.disableItem(f)}
					if(g.selected){this.selectItem(f)}return true
				}
				return false
			},
			removeItem:function(b,c){
				if(b==undefined||b==null){return}
				if(b.treeInstance!=undefined){b=b.element}
				var d=this;
				var e=b.id;
			...
			_getClosedSubtreeOffset:function(c){var b=a(c.subtreeElement);var e=-b.outerHeight();var d=-b.outerWidth();d=0;return{left:d,top:e}},_collapseItem:function(g,k,d,b){if(g==null||k==null){return false}if(k.disabled){return false}if(g.disabled){return false}if(g.locked){return false}var e=a(k.subtreeElement);var l=this._getClosedSubtreeOffset(k);var h=l.top;var c=l.left;$treeElement=a(k.element);var f=g.animationHideDelay;f=0;if(e.data("timer").show!=null){clearTimeout(e.data("timer").show);e.data("timer").show=null}var j=function(){k.isExpanded=false;if(g.checkboxes){var n=e.find(".chkbox");n.stop();n.css("opacity",1);e.find(".chkbox").animate({opacity:0},50)}
			var m=a(k.arrow);g._arrowStyle(m,"",k.isExpanded);e.slideUp(g.animationHideDuration,function(){k.isCollapsing=false;g._calculateWidth();var o=a(k.arrow);
			g._arrowStyle(o,"",k.isExpanded);e.hide();g._raiseEvent("1",{element:k.element})})};if(f>0){e.data("timer").hide=setTimeout(function(){j()},f)}else{j()}},
			_expandItem:function(g,k){if(g==null||k==null){return false}if(k.isExpanded){return false}if(k.locked){return false}if(k.disabled){return false}if(g.disabled){return false}var e=a(k.subtreeElement);if((e.data("timer"))!=null&&e.data("timer").hide!=null){clearTimeout(e.data("timer").hide)}var j=a(k.element);var h=0;var d=0;if(parseInt(e.css("top"))==h){k.isExpanded=true;return}
			var c=a(k.arrow);g._arrowStyle(c,"",k.isExpanded);if(g.checkboxes){var f=e.find(".chkbox");f.stop();f.css("opacity",0);f.animate({opacity:1},g.animationShowDuration)}e.slideDown(g.animationShowDuration,g.easing,function(){var l=a(k.arrow);k.isExpanded=true;g._arrowStyle(l,"",k.isExpanded);k.isExpanding=false;g._raiseEvent("0",{element:k.element});g._calculateWidth()});if(g.checkboxes){g._updateCheckItemLayout(k);if(k.subtreeElement){var b=a(k.subtreeElement).find("li");a.each(b,function(){var l=g.getItem(this);if(l!=null){g._updateCheckItemLayout(l)}})}}},
			_calculateWidth:function(){var e=this;var f=this.checkboxes?20:0;var d=0;a.each(this.items,function(){var g=a(this.element).height();if(g!=0){var k=this.titleElement.outerWidth()+10+f+(1+this.level)*20;d=Math.max(d,k);if(this.hasItems){var h=parseInt(a(this.titleElement).css("padding-top"));if(isNaN(h)){h=0}h=h*2;h+=2;var j=(h+a(this.titleElement).height())/2-17/2;if(a.jqx.browser.msie&&a.jqx.browser.version<9){a(this.arrow).css("margin-top","3px")}else{if(parseInt(j)>=0){a(this.arrow).css("margin-top",parseInt(j)+"px")}}}}});if(this.toggleIndicatorSize>16){d=d+this.toggleIndicatorSize-16}if(e.panel){if(d>this.host.width()){var b=d-this.host.width();e.panel.jqxPanel({horizontalScrollBarMax:b})}else{e.panel.jqxPanel({horizontalScrollBarMax:0})}}this.host.find("ul:first").width(d);var c=this.host.width()-30;if(c>0){this.host.find("ul:first").css("min-width",c)}if(e.panel){e.panel.jqxPanel("_arrange")}},
			_arrowStyle:function(c,h,b){
				var e=this;if(c.length>0){
					c.removeClass();var g="";
					if(h=="hover"){g="-"+h}
					var f=b?"-expand":"-collapse";var d="jqx-tree-item-arrow"+f+g;
			...
			_initialize:function(f,c){
				var e=this;
				var d=0;this.host.addClass(e.toThemeProperty("jqx-widget"));
				...
				a.each(this.items,function(){
					...
					if(!b){
						if(!m.hasItems){
							if(!e.rtl){m.element.style.marginLeft=parseInt(e.toggleIndicatorSize)+"px"}
							else{m.element.style.marginRight=parseInt(e.toggleIndicatorSize)+"px"}
							var j=a(m.arrow);
							if(j.length>0){
								j.remove();
								m.arrow=null;
						...
						else{m.element.style.marginLeft="0px"}
					}
					else{
						if(!m.hasItems&&a(m.element).find("ul").length>0){a(m.element).find("ul").remove()}
					}
					var j=a(m.arrow);
					if(j.length>0){
						j.remove()}
					k=a('<span style="height: 17px; border: none; background-color: transparent;" id="'+$element[0].id+'"></span>');
					//console.log(k);
					k.prependTo($element);
					if(!e.rtl){
						k.css("float","left")
					...
					if(h){
						...
						if(!m.isExpanded){
							e._expandItem(e,m)}
						else{e._collapseItem(e,m)}
						return false});
					e.addHandler(k,"selectstart",function(){return false});
					e.addHandler(k,"mouseup",function(){if(!h){return false}});
					if(!h){
						e.addHandler(k,"mouseenter",function(){
							e._arrowStyle(k,"hover",m.isExpanded)
						});
						e.addHandler(k,"mouseleave",function(){
							e._arrowStyle(k,"",m.isExpanded)})
					}
					m.hasItems=a(m.element).find("li").length>0;
					m.arrow=k[0];
					if(!m.hasItems){k.css("visibility","hidden")}
			...
			_updateDisabledState:function(){if(this.disabled){this.host.addClass(this.toThemeProperty("jqx-fill-state-disabled"))}else{this.host.removeClass(this.toThemeProperty("jqx-fill-state-disabled"))}},
			_addInput:function(){
				if(this.input==null){
					var b=this.host.attr("name");
					if(!b){b=this.element.id}
					else{this.host.attr("name","")}
			...
			_render:function(f,j){
				if(a.jqx.browser.msie&&a.jqx.browser.version<8){
					...
					a.each(this.items,function(){
						...
						if(o<parseInt(n.css("min-width"))){p.css("min-width",l)}
						this.titleElement[0].style.width=null})
				}
				var h=1000;var c=[5,5];
				var g=this;
				...
				if(f==undefined||f==true){
					a.each(this.items,function(){g._updateItemEvents(g,this)})}
				if(this.allowDrag&&this._enableDragDrop&&(j==undefined||j==true)){this._enableDragDrop()}
				this._addInput();
				if(this.host.jqxPanel){
					if(this.host.find("#panel"+this.element.id).length>0){
						this.panel.jqxPanel({touchMode:this.touchMode});
						this.panel.jqxPanel("refresh");return
					}
					...
					var k="fixed";
					if(this.height==null||this.height=="auto"){k="verticalwrap"}
					if(this.width==null||this.width=="auto"){if(k=="fixed"){k="horizontalwrap"}else{k="wrap"}}
					b.jqxPanel({rtl:this.rtl,theme:this.theme,width:"100%",height:"100%",touchMode:this.touchMode,sizeMode:k});
					if(a.jqx.browser.msie&&a.jqx.browser.version<8){b.jqxPanel("content").css("left","0px")}
					...
					}this.panelInstance=e;if(a.jqx.browser.msie&&a.jqx.browser.version<8){
			...
			_updateItemEvents:function(h,k){
				...
				var e=!b?"click":a.jqx.mobile.getTouchEventName("touchend");
				...
				h.addHandler(c,"selectstart",function(l){return false});
			...
			createID:function(){return a.jqx.utilities.createId()},
			createTree:function(b){
				if(b==null){return}
				var d=this;var f=a(b).find("li");
				var c=0;
				this.items=new Array();
				this.itemMapping=new Array();
				a(b).addClass(d.toThemeProperty("jqx-tree-dropdown-root"));
				if(this.rtl){a(b).addClass(d.toThemeProperty("jqx-tree-dropdown-root-rtl"))}
				...
				if(a.jqx.browser.msie&&a.jqx.browser.version<8){}
				for(var e=0;e<f.length;e++){ this._createItem(f[e]) }
				if(this.rtl||a.jqx.browser.msie&&a.jqx.browser.version<8){
				...
				if(this.checkboxes){
					if(a(b.titleElement).css("display")!="none"){
						...
						if(!this.rtl){
							...
							else{if(b.hasItems){c.css("margin-left",this.toggleIndicatorSize)}}}}
					}
			},
			...
			_updateCheckState:function(e){
				...
				if(b>0){
					if(c==b){this.checkItem(e.element,true,"tree")}
					else{
						if(c>0){
							this.checkItem(e.element,null,"tree");
						}else{this.checkItem(
							e.element,false,"tree");
						}
					}
				}
			},
			...
			_createItem:function(c){
				...
				if(r==null||r==undefined||(r!="true"&&r!=true)){
					...
					if(j==null||j==undefined||(j!="true"&&j!=true)){j=false}
					else{j=true}
					...
					if(!h||!D){
						if(a(E.firstChild).length>0){
							...
							if(b==-1){
								B.originalTitle=E.innerHTML;E.innerHTML='<div style="display: inline-block;">'+E.innerHTML+"</div>";
								...
								a(E).append(n);
								B.titleElement=A;
								if(this.rtl){A.css("float","right")}
							}
							if(a.jqx.browser.msie&&a.jqx.browser.version<8){
									a(a(E)[0].firstChild).css("display","inline-block");
									this._measureItem.html(a(B.titleElement).text());
									var t=5+this._measureItem.outerWidth();
									if(B.icon){t+=20}
									if(this.checkboxes){t+=20; }
									a(a(E)[0].firstChild).css("max-width",t+"px");
							}
						}
						else{B.originalTitle="Item";
				...
				if(this.checkboxes){
					if(this.host.jqxCheckBox){
						...
						o.jqxCheckBox({hasInput:false,checked:B.checked,boxSize:this.checkSize,animationShowDelay:0,animationHideDelay:0,disabled:d,theme:this.theme});
						if(!this.rtl){
							y.css("margin-left",parseInt(this.checkSize)+6)}
						else{
							var x=5;
							if(B.parentElement){
				...
				if(d){this.disableItem(B.element)}
				if(r){this.selectItem(B.element)}
				if(a.jqx.browser.msie&&a.jqx.browser.version<8){
					a(E).css("margin","0px");
					a(E).css("padding","0px");
				}
			...
			propertyChangedHandler:function(d,l,b,j){
				...
				if(l=="submitCheckedItems"){d._updateInputSelection()}
				if(l=="disabled"){d._updateDisabledState()}
		...
		(function(a){
			a.jqx._jqxTree.jqxTreeItem=function(e,d,b){
				var c={
					label:null, id:e, parentId:d, parentElement:null, parentItem:null,
					disabled:false,selected:false,locked:false,checked:false,level:0,
					isExpanded:false,hasItems:false,element:null,subtreeElement:null,
					checkBoxElement:null,titleElement:null,arrow:null,prevItem:null,nextItem:null
				};
				return c
			}
		})(jQuery);
